                ...
                data_buf
                    ...
                    .map(|p| {
                        ...
                        let convert = |mask: u32| -> u8 {
                            ((pixel & mask)
                                >> mask.trailing_zeros() * 255 / (2u32.pow(mask.count_ones()) - 1))
                                as u8
                        };
                ...
                (0..4)
                    .map(|j| {
                        ...
                        let alpha = match (
                            ...
                            (false, 6) => 0,
                            (false, 7) => 255,
                            t => unreachable!("This value should not have occurred: `{:?}`", t),
                        };
    ...
    fn decode_dxt(header: &Header, data_buf: &mut Vec<u8>) -> Vec<Vec<RGBA<u8>>> {
        ...
        layer_sizes
            ...
            .map(|&(height, width)| {
                ...
                let chunk_transform: Box<dyn Fn(&[u8]) -> Vec<RGBA<u8>>> = match header.compression
                {
                    Compression::DXT1 => Box::new(|chunk| {
                        DDS::bytes_to_pixels_dxt1(
                            chunk,
                            header.pixel_format != PixelFormat::Unknown,
                        )
                    }),
